...
export async function POST(req: Request) {
  try {
    ...
    for (let i = 0; i < dates.length; i++) {
      ...
      const diffs: AccuracyDiffs = {
        swellHeight: diff(actual?.swellHeight ?? null, predicted?.swellHeight ?? null),
        swellPeriod: diff(actual?.swellPeriod ?? null, predicted?.swellPeriod ?? null),
        waveHeight: diff(actual?.waveHeight ?? null, predicted?.waveHeight ?? null),
        waterTemperature: diff(actual?.waterTemperature ?? null, predicted?.waterTemperature ?? null),
        windSpeed: diff(actual?.windSpeed ?? null, predicted?.windSpeed ?? null),
        windDirection: diff(actual?.windDirection ?? null, predicted?.windDirection ?? null),
        tideHigh: diff(actual?.tideHigh ?? null, predicted?.tideHigh ?? null),
        tideLow: diff(actual?.tideLow ?? null, predicted?.tideLow ?? null),
      };

      days.push({ date: d, actual, predicted, diffs });
    }

    const payload: AccuracyResponse = { lat: rl, lon: rlo, centerDate: cd, days };
