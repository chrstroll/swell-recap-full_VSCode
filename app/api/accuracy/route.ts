...
// Direction error normalized to [-180, 180]
function diffAngle(a: number | null, b: number | null): number | null {
  if (a == null || b == null) return null;
  let diff = a - b;
...
export async function POST(req: Request) {
  try {
    ...
    // 2) Predictions from marine + forecast APIs
    ...
    if (forecastRes.ok) {
      ...
      console.warn(
        "[accuracy] forecast wind API error",
        forecastRes.status,
    ...
    const days: AccuracyDay[] = dates.map((d) => {
      ...
      return {
        ...
        diff: {
          ...
          windSpeed: diffNumber(
            actual?.wind.speed ?? null,
            predicted?.wind.speed ?? null
          ),
          windDirection: diffAngle(
            actual?.wind.direction ?? null,
            predicted?.wind.direction ?? null
          ),
          tideHigh: diffNumber(
